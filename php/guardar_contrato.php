...
function post_decimal_null(string $key): ?float
{
    $value = post_str($key, '');
...
try {
    ...
    $ubicacionLat = post_decimal_null('ubicacion_lat');
    $ubicacionLng = post_decimal_null('ubicacion_lng');
    $ubicacionPrecision = post_decimal_null('ubicacion_precision');
    $ubicacionFuente = post_optional_null('ubicacion_fuente');

    if (($ubicacionLat === null && $ubicacionLng !== null) || ($ubicacionLat !== null && $ubicacionLng === null)) {
        throw new Exception('La ubicación está incompleta. Debe incluir latitud y longitud.');
    }

    // Solo guardar fecha de ubicación si realmente viene una coordenada válida
    $ubicacionFecha = ($ubicacionLat !== null && $ubicacionLng !== null)
        ? date('Y-m-d H:i:s')
        : null;
    ...
    $data = [
        ...
        'ubicacion_fecha' => $ubicacionFecha,
